...
public class TrieSET implements Iterable<String> {
     ...
     * the keys in a set named <tt>set</tt>, use the foreach notation:
    ...
    public Iterable<String> keysWithPrefix(String prefix) {
        Qpueue<String> results = new Qpueue<String>();
        Node x = get(root, prefix, 0);
        collect(x, new StringBuilder(prefix), results);
        return results;
    }

    private void collect(Node x, StringBuilder prefix, Qpueue<String> results) {
        if (x == null)
            return;
        if (x.isString)
    ...
    private void collect(Node x, StringBuilder prefix, String pattern, Qpueue<String> results) {
        if (x == null)
            return;
        int d = prefix.length();
